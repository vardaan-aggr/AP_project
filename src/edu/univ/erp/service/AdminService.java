...
public class AdminService {
    ...
    public String deleteCourseAndSection(String courseCode, String section) {
        ...
        switch (result) {
            case 1:
                return "Success: Course and Section deleted successfully.";
            case 2:
                return "Warning: Section deleted, but Course not found in database.";
            case 3:
                return "Warning: Course deleted, but Section not found in database.";
            case 4:
                return "Error: Neither the Course nor the Section existed.";
            case -1:
                return "Error: Database failed to delete Section.";
            case -2:
                return "Error: Database failed to delete Course.";
            case -3:
    ...
    public String performBackup(String savePath) {
        ...
        try {
            ...
            return (exitCode == 0) ? "Success" : "Error: Backup failed (Code " + exitCode + ")";
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public String performRestore(String loadPath) {
        ...
        try {
            Process p = pb.start();
            int exitCode = p.waitFor();
            return (exitCode == 0) ? "Success" : "Error: Restore failed (Code " + exitCode + ")";
        } catch (Exception e) {
            e.printStackTrace();
            return "Error: " + e.getMessage();
        }
    ...
    public ArrayList<Student> getAllStudents() throws SQLException {
        ...
        // 2. If list is not empty, fill details from ERP DB
        if (!students.isEmpty()) {
            ErpCommandRunner.searchStudentErp(students);
        }
    ...
    public ArrayList<Instructor> getAllInstructors() throws SQLException {
        // 1. Get basic list from Auth DB
        ArrayList<Instructor> instructors = AuthCommandRunner.searchInstructorAuth();

        // 2. Fill details from ERP DB
        if (!instructors.isEmpty()) {
            ErpCommandRunner.searchInstructorErp(instructors);
        }
